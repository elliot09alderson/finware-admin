...
const AddProduct = () => {
  ...
  const { selectedSubCategories } = useSelector((state) => state.subCategory);
  const [catId, setCatId] = useState("");

  console.log(selectedSubCategories, "selected sub categories");
  ...
  useEffect(() => {
    if (catId) {
      ...
      dispatch(get_selected_sub_category(catId));
    }
  }, [catId]);
  ...
  //   } else {
  //     setAllSubCategory(subcategorys);
  //   }
  // };
        ...
        <div>
          <form onSubmit={add}>
            ...
            <div className="flex flex-col mb-3 md:flex-row gap-4 w-full text-[#d0d2d6]">
              <div className="flex flex-col w-full gap-1 relative">
                ...
                <input
                  ...
                  className="px-4 py-2 focus:border-indigo-500 outline-none bg-[#283046] border border-slate-700 rounded-md text-[#d0d2d6]"
                  onChange={(e) => inputHandle(e, category._id)}
                  value={category.name}
                  type="text"
              ...
              {selectedSubCategories.length > 0 && (
                <div className="flex flex-col w-full gap-1 relative">
                  ...
                  >
                    ...
                    <div className="pt-14"></div>
                    <div className="flex justify-start items-start flex-col h-[200px] overflow-x-scroll">
                      {/* mapping of Sb Categories  */}
                      {selectedSubCategories[0].map((c, i) => (
                        <span
                          key={i + c._id}
                          className={`px-4 py-2 hover:bg-indigo-500 hover:text-white hover:shadow-lg w-full cursor-pointer ${
                            subcategory.name === c.name && "bg-indigo-500"
                          }`}
                          onClick={() => {
            ...
            <div className="grid lg:grid-cols-4 grid-cols-1 md:grid-cols-3 sm:grid-cols-2 sm:gap-4 md:gap-4 xs:gap-4 gap-3 w-full text-[#d0d2d6] mb-4">
              {imageShow.map((img, i) => (
                <div className="h-[180px] relative" key={i + 22}>
                  <label htmlFor={i}>
                    <img
